...
class PeripheralPersonPerformanceController {
	...
	def list = {
		...
		[personalPerformanceInstanceList: personalPerformanceService.findAllPeripheralPersonalPerformanceByUser(jobService.getCodesByUser(currentUser),params), personalPerformanceInstanceTotal: PersonalPerformance.count()]
	}
	...
	def show = {
		...
		if (!personalPerformanceInstance) {
			...
			if(peripheralPeople!=null&&peripheralPeople.length()>0){
				...
				jobService.getCodesByUser(currentUser).each { jobCode->
					if(jobCode in codes){
