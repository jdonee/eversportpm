	...
	def init = { servletContext ->
		switch(GrailsUtil.environment){
			case "development":
				...
				def testUser1 = new User(username: 'frank', employeeName:'曾爱辉',enabled: true, password: springSecurityService.encodePassword('frank'))
				testUser1.save(flush: true)
				def testUser2 = new User(username: 'tina',employeeName:'沈晓婷', enabled: true, password: springSecurityService.encodePassword('tina'))
				testUser2.save(flush: true)
				def testUser3 = new User(username: 'alice', employeeName:'沈婷婷',enabled: true, password: springSecurityService.encodePassword('alice'))
				testUser3.save(flush: true)
				def testUser4 = new User(username: 'joy',employeeName:'沈小晓', enabled: true, password: springSecurityService.encodePassword('joy'))
				testUser4.save(flush: true)
				def testUser5 = new User(username: 'joyce', employeeName:'沈娟娟',enabled: true, password: springSecurityService.encodePassword('joyce'))
				testUser5.save(flush: true)
				def testUser6 = new User(username: 'nancy',employeeName:'沈凤燕', enabled: true, password: springSecurityService.encodePassword('nancy'))
				testUser6.save(flush: true)
				def testUser7 = new User(username: 'jane', employeeName:'沈雯雯',enabled: true, password: springSecurityService.encodePassword('jane'))
				testUser7.save(flush: true)
				def testUser8 = new User(username: 'lori',employeeName:'沈梦琪', enabled: true, password: springSecurityService.encodePassword('lori'))
